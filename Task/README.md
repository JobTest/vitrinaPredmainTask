...
> Транзакция net.sf.hibernate.Transaction – однопоточный, короткоживущий объект, используемый приложением для указания атомарной единицы выполняемой работы.
(Session – однопоточный, короткоживущий объект, являющийся посредником между приложением и хранилищем долгоживущих объектов)
* Долгоживущие Объекты и Коллекции (Persistent Objects and Collections) – это
  однопоточные, короткоживущие объекты, содержащие сохраняемое состояние
  и бизнес-функции. Это могут быть обычные JavaBean/POJO (Plain Old Java
  Objects) объекты, их отличительная особенность – это то, что они ассоциированы
  с одной сессией (Session). Как только их сессия закрыта, эти объекты становят-
  ся отсоединенными и свободными для использования на любом уровне приложе-
  ния, например, непосредственно как объекты передачи данных на уровень пред-
  ставления и с него.
...
> Напримере можно взять простую задачу:
  - есть (сущность) 'школа'
  - (сущность) 'учителя'
  - (сущность) 'ученики'
  - (сущность) 'классы'
  -- (сущность) 'учебная программа'
  -- (сущность) 'экзаменционные тесты'
  Центральным игроком здесь можно отметить (сущность) 'класс':
  Группа учеников переходят в класс (№7) для прохождения учебной программы за 7-класс, которую будет вести учитель 7-класса и в итоге знания учеников будут оцениваться тестами за 7-класс.
  Предварительно:
  - (сущность) 'класс' может создаваться максимум несколько раз за все время существования школы.
  - (сущность) 'ученик' моздается 1-раз в 10-лет
  -- но история успеваемости учеников обновляется/пополняется каждый год, при условии что каждый год ученики выпускники/уходят из школы.
  То есть, после удаления ученика, каскадно, удаляются и все ссылки которые с ним были связаны !
  А что если требуется хранить историю вечно, которая ссылается на этого ученика ?
  #1 В этом случае (сущность) 'класс' можно привязыват к году выпуска, а сами ссылки на этих учеников удалять.
  #2 Или можно создавать схему для таблиц (потом ограничивать доступ к ней по году выпуска), а саму базу привязывать к году выпуска.
  #3 Или же просто в (сущность) 'класс' добавить такую-себе заглушку, куда попадают уже ужедшие выпускники...
