...
@Table(name = "issue")
@NamedQueries({
        @NamedQuery(name = "Issue3.findAll", query = "SELECT issue FROM Issue3 issue"),
        @NamedQuery(name = "Issue3.findByProject", query = "SELECT issue FROM Issue3 issue WHERE issue.ProjectName = :ProjectName"),
        @NamedQuery(name = "Issue3.findByTracker", query = "SELECT issue FROM Issue3 issue WHERE issue.TrackerName = :TrackerName")
})
public class Issue3 {
...
//        if (this == o) return true;
        if (!(o instanceof Issue3))
            return false;

        Issue3 issue = (Issue3) o;
        if (Id != issue.Id)
            return false;
